...
@objc protocol OrderGoodsListViewDelegate: NSObjectProtocol {
...
class OrderGoodsListView: UIView {
    ...
    override func layoutSubviews() {
        ...
        if delegate != nil && (self.delegate?.responds(to: #selector(OrderGoodsListViewDelegate.orderGoodsListViewHeightDidChange(height:))))! {
        delegate!.orderGoodsListViewHeightDidChange( height: payLabel.frame.maxY + 10)
        }
    ...
    var order_goods: [[OrderGoods]]? {
        didSet {
            for i in 0..<order_goods!.count {
                ...
                if array.count == 1 {
                    let goodsView = GoodsView(frame: CGRect(x:0, y:lastViewY + 10, width:ZJScreenWidth, height:20))
                    ...
                    for i in 0..<array.count {
                        ...
                        if i == 0 {
                            goodsView.frame = CGRect(x:0, y:lastViewY + 10, width:ZJScreenWidth, height:20)
                            lastViewY = goodsView.frame.maxY
                        } else {
                            goodsView.frame = CGRect(x:0, y:lastViewY, width:ZJScreenWidth, height:20)
                            lastViewY = goodsView.frame.maxY + 10
                        }
                ...
                lineView.frame = CGRect(x:10, y:orderGoodsViewHeight, width:ZJScreenWidth - 10, height:1)
                addSubview(lineView)
            }
        }
    }
...
class GoodsView: UIView {
    ...
    var orderGoods: OrderGoods? {
        didSet {
            ...
            priceLabel.text = "$" + (orderGoods?.goods_price)!.cleanDecimalPointZero()
        ...
        didSet {
            if (fee_list?.count)! > 1 {
                for i in 0..<fee_list!.count {
                    let feelView = FeeView(frame: CGRect(x:0, y:10 + CGFloat(i) * 25, width:ZJScreenWidth, height:25), fee: fee_list![i])
...
class FeeView: UIView {
    ...
    convenience init(frame: CGRect, fee: OrderFeeList) {
        ...
        prictLabel.text = "$" + (fee.value?.cleanDecimalPointZero())!
    }
